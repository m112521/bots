...
// The ESP32 will create its own WiFi network with these credentials
const char* ssid = "bubblebot";
...
// ============================================================
// KEY CHANGE: ESP32 now creates its own WiFi Access Point
// instead of connecting to an existing network as a client.
// ============================================================
void initWiFi() {
  // Set ESP32 as Access Point (AP) instead of Station (STA)
  WiFi.mode(WIFI_AP);
  
  // Start the soft AP with SSID and password
  // Remove password parameter or set it to NULL for an open network
  WiFi.softAP(ssid, password);

  // Optional: print the AP IP address (always 192.168.4.1 by default)
...
void setup()
{
  ...
  initWiFi();      // Now starts as AP instead of connecting to a router
  initWebSocket();
