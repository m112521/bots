...
<style>
    ...
    .parent {
        ...
        .btn-set{width: 35%;border-radius:0 15px 15px 0;border:none;background:#524FF0;color:white;font-size:1.3rem}
        ...
        .slider{width: 90%;}
        .circ-btn{border-radius:100%;border:1px solid red;width:50%;height:100%;background:none;}
</style>
<div class="parent">
    <div class="div1">
        <div class="parent-j">
    ...
    <div class="div2">
        <div class="analog">
            <div class="analog-data"><div class="analog-value" id="slider-txt">0</div>
                <div class="div-slider"><input id="slider-val" class="slider" type="range" min="0" max="255"/></div>
            </div> 
            <button class="btn-set" id="btn-set">SET</button>
        </div>
    </div>
    <div class="div3"><button class="circ-btn" id="btn-fire">FIRE!</button><button class="circ-btn" id="btn-led">LED</button></div>
</div>
...
function initButtons() {
  ...
  document.querySelector('#btn-fire').addEventListener('click', ()=>{ websocket.send(JSON.stringify({fire:fireState})) });
  document.querySelector('#btn-led').addEventListener('click', ()=>{ websocket.send(JSON.stringify({led:ledState})) });
}
...
function onMessage(event) {
    ...
    //document.querySelector("#slider-txt").innerHTML = myObj["s"];
    websocket.send("getReadings");
}</script>
)rawliteral";
